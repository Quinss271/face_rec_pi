...
def recog():
    ...
    for x in range(len(res)):
        ...
        known_face_encodings[x] = face_recognition.face_encodings(x_image)[0]



    mycursor = mydb.cursor()
    sql = "SELECT `fio` FROM `verify_pi`"
    fio= mycursor.execute(sql)
    fio=mycursor.fetchall()
    print(fio)
    known_face_names = fio
